...
1. Suppose we have the following survival data...  
  ...
  b. Write out the partial likelihood function as a function of the regression coefficient.  
    PL(B) = exp(2.28B) / [exp(2.6B) + exp(2.85B) + exp(2.74B) + exp(2.34B) + exp(3.05B)] +  
    exp(2.85B) / [exp(2.74B) + exp(2.34B) + exp(3.05B)] +  
    exp(2.74B) / [exp(2.34B) + exp(3.05B)] +  
    exp(3.05B) / [exp(3.05B)]  
...
2. The US Veterans Administration...  
  ...
  c. What are the key assumptions assumed in the Cox PH model in (b)?  
    The key assumption is that the baseline hazard rate is common to all patients and that the covariates' effect on hazard rate doesn't vary with time.  
  ...
  e. Do patients in standard chemotherapy have longer than expected survival time compared with patients in test chemotherapy? Explain.  
    The estimated coefficient for Trt is > 0, and hence the relative hazard rate is higher for patients in test chemotherapy. exp(1.0672) = 2.91, so the hazard rate of a patient with standard chemotherapy is 2.91 times the hazard rate of a patient with test chemotherapy, hence the survival time is lower for a patient with standard chemotherapy.  
...
```
  ...
  d. Consider a Cox proportional hazard model with only Race as covariate. What is the MPLE of the hazard ratio comparing "blacks" to "Hispanics" and interpret the regression coefficient of Race.  
    The MPLE coefficient of Black with respect to Hispanic is -0.4159, for a hazard rate ratio of 0.66. In other words, a black patient's hazard rate is always 66% that of a hispanic patients.  
```{r, echo=FALSE, message=FALSE, warning=FALSE}
